...
- (void)checkTimer
{
    if (!_displayLink) {
        ...
        _oldTime = CACurrentMediaTime();
    }else if (_displayLink.paused) {
        _oldTime = CACurrentMediaTime();
    }
...
+ (void)stopAll {
    ...
    while ([manager tweens].count) {
        [manager removeTween:manager.tweens.firstObject];
    }
}

+ (void)cancel:(id)object {
    GTweenManager *manager = [GTweenManager instance];
    NSArray *arr = [manager tweens];
    for (GTween *tween in arr) {
